...
function createSolutions($df) {
    ...
    foreach ($output as $key => $val) { // Gibt alle Möglichkeiten der Reihenfolge als String wieder
        ...
        $current_result = implode($result);
        for ($ei=5; $ei < strlen($current_result);$ei++) { // get substring
            for ($offset=0; $offset < (strlen($current_result)-$ei);$offset++) { // get offset
                $cc = substr($current_result, $offset, $ei);
                if ($cc == strrev($cc)) {
                    return "IMPOSSIBLE";
                }
            }
